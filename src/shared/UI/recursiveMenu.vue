<template>
    <a href="javascript:;" class="side-menu__item" :class="`${menuData?.selected ? 'active' : ''}`"
        @mouseover="HoverToggleInnerMenuFn($event, menuData)" @click="toggleSubmenu($event, menuData, undefined, level)">
        <span class="shape1" v-if="level == 1"></span><span v-if="level == 1" class="shape2"></span>
        ...
        <li v-for="(firstLevelMenuItem, subIndex) in menuData.children" :key="subIndex"
            ...
            <template v-if="firstLevelMenuItem?.type === 'empty'">
                <a href="javascript:;" class="side-menu__item">
                    ...
                    <span v-if="firstLevelMenuItem.badge" :class="`badge ${firstLevelMenuItem.badgeColor} ms-1`">{{
