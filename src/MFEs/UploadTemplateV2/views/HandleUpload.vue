    ...
    <div class="bg-white py-4 px-5 col col-12 card">
      <form-wizard
        ...
        <tab-content title="Resultado">
          <div class="col-xs-12 text-start pb-4">
            <div class="col-md-12">
              <template v-if="store.proccessResult.data.status === 'ERROR'">
                ...
                <div
                  ...
                  role="alert">
                  {{ store.processStatus.data.errorDescription }}
                </div>

                <template v-if="store.proccessResult.data.errors">
                  <p>
                    Revisa los siguientes puntos:
                  </p>
                  <ul>
                    <li
                      v-for="error in store.proccessResult.data.errors.split(',')"
                      :key="error"
                      class="mb-2">
                      {{ error }}
                    </li>
...
const uploadFile = async () => {
  ...
  if (!idProcess) {
    console.error("No se pudo obtener idProcess")

    return false
  }

  // Polling para esperar hasta que el proceso termine
  let attempts = 0
  const maxAttempts = 10 // Límite de intentos
  const delay = 5000 // 5 segundos entre intentos

  while (attempts < maxAttempts) {
    await store.getProcessStatus()

    const status = (store.processStatus.data as { status?: string })?.status

    if (status === "SUCCESS" || status === "ERROR") {
      await store.getProcessByCompanyId(companySelected.value.id)
  ...
  console.error("El proceso no terminó después de varios intentos.")
